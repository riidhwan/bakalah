...
`LocalVaultChapterStager` owns Local-to-Vault directory chapter staging and CBZ preparation. It converts directory chapters into validated flat numbered CBZ files in app-managed temporary storage, preserves original Local Source files, updates the staged chapter's content metadata, and leaves already-CBZ chapters unchanged. `LocalVaultChapterPublisher` owns the Local-to-Vault one-chapter publish boundary: remote manifest reads and identity validation, Local-to-Vault duplicate replacement checks, staging invocation, chapter and initial-cover upload, manga/root manifest writes, rollback after root publish failure, best-effort old content cleanup, replacement cache invalidation, and per-chapter publish result mapping. `VaultWebDavClient` owns low-level WebDAV transport mechanics only and preserves nullable and boolean transport results; it does not own manifest validation, revision policy, rollback policy, refresh timing, or result semantics. `LocalVaultImportService` remains the workflow coordinator that owns preview/import sequencing, selected-chapter handling, progress, cancellation, transfer job result mapping, per-success catalogue refresh, target state, and Import Target Hint persistence.

Implementation debt: keep Local-to-Vault Import and Library-to-Vault Capture publishing workflow-specific until both one-chapter publisher boundaries have enough tests to prove their semantics are actually aligned. `LocalVaultChapterPublisher` and `LibraryVaultChapterPublisher` currently stay separate and keep separate active-target, publish-result, and workflow failure types. `LibraryVaultChapterPublisher` owns Capture-specific upload, manifest mutation, rollback, target validation, and replacement cache invalidation, while `LibraryVaultChapterStager` owns Capture-specific downloaded-page copy/extraction, source page fetching, tall-image splitting, canonical CBZ writing, validation, and cover acquisition. Capture should not consume the local-import stager until shared lower-level page-to-CBZ mechanics can be extracted without merging workflow-specific source/download acquisition policy. Capture uses `LibraryVaultCaptureWebDav` as its workflow-specific transport wrapper; adopting `VaultWebDavClient` is deferred until transport behavior can be verified independently from capture publishing behavior. Shared manifest publishing mechanics should wait until tests cover both one-chapter publisher boundaries and scanner/stager responsibilities. Shared refresh timing should be revisited only after tests prove whether per-chapter refresh or batched refresh is required for each workflow. Full publisher tests currently cover successful create-new publish, replacement publish with cache invalidation, unconfirmed duplicates as per-chapter failures, root publish rollback, root identity mismatch, missing target pointer, and no empty Vault Manga after pre-publish failures; future sharing should preserve those semantics. `LocalVaultImportResult` and `LibraryVaultCaptureResult` cleanup should wait until Import and Capture result semantics are aligned.
...
Partial success is expected. If at least one chapter is added or replaced, the job can finish as `PARTIALLY_SUCCEEDED` when other chapters fail. If every chapter fails, no empty Vault Manga is created. For Create New imports, the first successfully imported chapter creates the Vault Manga; the first selected chapter is not required to be the first success. Per-chapter failures include missing selected chapters, staging or CBZ validation failures, unconfirmed duplicates, chapter-specific read failures, and content upload failures before manifest publish. Global failures such as configured identity changes, target deletion, unreadable root manifests, root publish failure after a manga manifest write, credentials failure, or user cancellation stop remaining work.

Accepted Add to Vault actions are persisted as Vault Import Requests before WorkManager is enqueued. A request stores the source manga, workflow, Import Target, and selected chapters as durable child rows with each chapter's database id when available, workflow selection id, original order, and whether that selected chapter may become a Vault Chapter Replacement. WorkManager input carries only the request id; the worker reloads the request and current manga/chapter rows before calling the import or capture service. Missing selected chapters are handled by the workflow result path for both workflows and recorded as per-chapter failures when at least one selected chapter can still be processed. Terminal requests are deleted or pruned after their result has been represented in the workflow result trail.

Implementation debt: after Local-to-Vault Import and Library-to-Vault Capture expose matching result semantics, extract the duplicated WorkManager/request plumbing from `LocalVaultImportJob` and `LibraryVaultCaptureJob` into a shared Add to Vault job runner or delegate. The shared runner should own request loading, workflow validation, foreground setup, running guards, request creation, notification lifecycle hooks, and terminal request cleanup while keeping workflow-specific service calls and result-to-notification mapping explicit.
...
Implementation debt: after Local-to-Vault Import and Library-to-Vault Capture both use one-chapter publish workflows with visible transfer results, extract the duplicated app-layer publishing mechanics into shared helpers or services. The shared code should cover root manifest reads and identity validation, manga manifest path resolution, content path creation, staged upload and promotion, root pointer updates, rollback after root publish failure, local index refresh, transfer job finalization, and sanitized failure detail encoding while keeping workflow-specific policy outside the helper.
