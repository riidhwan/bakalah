...
Implementation debt: after Local-to-Vault Import and Library-to-Vault Capture both use one-chapter publish workflows with visible transfer results, continue extracting duplicated app-layer publishing mechanics into shared helpers or services. Root manifest reads and identity validation, manga manifest path resolution, root pointer updates, and rollback after root publish failure are already shared for the two one-chapter publishers. The remaining shared code candidates include content path creation, staged upload and promotion, local index refresh, transfer job finalization, and sanitized failure detail encoding while keeping workflow-specific policy outside the helper.
