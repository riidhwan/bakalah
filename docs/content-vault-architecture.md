...
- `LocalVaultImportService`: scans Local Source manga, plans duplicates and target selection, stages selected directory chapters as CBZ without mutating Local Source files, uploads chapter and cover content, publishes manifests, refreshes the index, writes import target hints, and records visible `IMPORT_PUBLISH` results.
...
Implementation debt: after Local-to-Vault Import behavior is brought to parity, extract the duplicated domain planning mechanics shared by `BuildLocalVaultImportPlan` and `BuildLibraryVaultCapturePlan`. The shared helper should cover guarded Import Target Hint resolution, exact normalized title matching, explicit target choice, create-new fallback, and default duplicate deselection while leaving workflow-specific chapter shape and duplicate key policy separate. `MangaScreenModel` duplicate indicators should then consume the same planning result or shared duplicate-key policy so UI indicators and worker behavior cannot drift.
...
Implementation debt: `LocalVaultImportService` currently concentrates orchestration plus Local Source scanning, chapter staging, CBZ construction, WebDAV publishing, manifest mutation, rollback, cache invalidation, transfer result accounting, ordering, digesting, and failure-detail serialization. Cleanup should be behavior-preserving extraction first, not a semantic redesign. The first pass should extract app-local pure helper files for WebDAV paths, CBZ names, import ordering, and failure detail encoding, then extract a `LocalVaultMangaScanner` that returns app-layer scan models containing `UniFile` plus the domain planning models needed by `BuildLocalVaultImportPlan`. The second pass should extract chapter staging/CBZ preparation into a focused stager. A later pass should extract shared manifest publishing mechanics only after tests cover the scanner and stager boundaries. Keep `LocalVaultImportService` as the workflow coordinator that owns preview/import sequencing, progress, cancellation, target state, and result mapping.
