...
Implementation debt: keep Local-to-Vault publishing workflow-specific until Library-to-Vault Capture has matching one-chapter publish result semantics. Defer typed publisher results until tests cover the extracted `LocalVaultChapterPublisher` boundary. Library-to-Vault Capture should not consume the local-import stager until shared lower-level page-to-CBZ mechanics can be extracted without merging workflow-specific source/download acquisition policy. Library-to-Vault Capture may adopt `VaultWebDavClient` only after the Local-to-Vault extraction remains green and the change can be verified separately from capture publishing behavior. Shared manifest publishing mechanics should wait until tests cover scanner and stager boundaries and both Local-to-Vault Import and Library-to-Vault Capture expose matching one-chapter publish result semantics. Shared refresh timing should be revisited only after tests prove whether per-chapter refresh or batched refresh is required for each workflow. Full publisher integration tests should cover successful create-new publish, replacement publish with cache invalidation, unconfirmed duplicates, root publish rollback, and root identity mismatch. `LocalVaultImportResult` and `LibraryVaultCaptureResult` cleanup should wait until Import and Capture result semantics are aligned.
