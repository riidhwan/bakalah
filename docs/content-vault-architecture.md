...
Vault Operation Notifications make this queue visible while preserving Vault privacy. The operation worker runs as foreground WorkManager work while it drains Optimistic Background Publish handlers. Waking the queue replaces stale non-running unique WorkManager work, but keeps a currently running drain worker so a live publish is not cancelled just because another short operation was accepted. It shows one queue-level, indeterminate, ongoing notification with generic status text such as publishing vault changes, saving metadata, deleting a chapter, or refreshing the vault. The notification has no cancel action because running manifest publishes do not yet have user-cancellation semantics. All-success queue completion quietly removes the running notification. Failed or partially successful completion leaves one dismissible aggregate failure notification without manga titles, chapter titles, remote paths, or raw internal failure reasons. Tapping that failure notification opens Bakalah normally rather than deep-linking to a specific Vault Manga or operation detail.
