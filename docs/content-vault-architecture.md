...
Implementation debt: `LocalVaultImportService` still concentrates orchestration plus chapter staging, CBZ construction, WebDAV publishing, manifest mutation, rollback, cache invalidation, and transfer result accounting. App-local helper files already own WebDAV path construction, CBZ naming and validation, import ordering, file digesting, failure detail encoding, and Local Source scanning through `LocalVaultMangaScanner`. The next behavior-preserving pass should extract chapter staging/CBZ preparation into a focused stager. A later pass should extract shared manifest publishing mechanics only after tests cover the scanner and stager boundaries. Keep `LocalVaultImportService` as the workflow coordinator that owns preview/import sequencing, progress, cancellation, target state, and result mapping.
