...
`LocalVaultChapterStager` owns Local-to-Vault directory chapter staging and CBZ preparation. It converts directory chapters into validated flat numbered CBZ files in app-managed temporary storage, preserves original Local Source files, updates the staged chapter's content metadata, and leaves already-CBZ chapters unchanged. `LocalVaultImportService` remains the workflow coordinator that owns preview/import sequencing, progress, cancellation, target state, WebDAV publishing, manifest mutation, rollback, cache invalidation, and result mapping. Implementation debt: Library-to-Vault Capture should not consume the local-import stager until shared lower-level page-to-CBZ mechanics can be extracted without merging workflow-specific source/download acquisition policy. A later pass should extract shared manifest publishing mechanics only after tests cover the scanner and stager boundaries.
