...
`LocalVaultChapterStager` owns Local-to-Vault directory chapter staging and CBZ preparation. It converts directory chapters into validated flat numbered CBZ files in app-managed temporary storage, preserves original Local Source files, updates the staged chapter's content metadata, and leaves already-CBZ chapters unchanged. `LocalVaultChapterPublisher` owns the Local-to-Vault one-chapter publish boundary: remote manifest reads and identity validation, Local-to-Vault duplicate replacement checks, staging invocation, chapter and initial-cover upload, manga/root manifest writes, rollback after root publish failure, best-effort old content cleanup, replacement cache invalidation, and per-chapter publish result mapping. Prepared chapter and cover upload path creation is shared through `VaultContentUploader`, which depends on a narrow `VaultContentUploadStorage` port implemented by workflow-specific WebDAV wrappers. `VaultWebDavClient` owns low-level WebDAV transport mechanics only and preserves nullable and boolean transport results; it does not own manifest validation, revision policy, rollback policy, refresh timing, or result semantics. `LocalVaultImportService` remains the workflow coordinator that owns preview/import sequencing, selected-chapter handling, progress, cancellation, transfer job result mapping, per-success catalogue refresh, target state, and Import Target Hint persistence.

Implementation debt: keep Local-to-Vault Import and Library-to-Vault Capture publishing workflow-specific until their finer-grained semantics are actually aligned. `LocalVaultChapterPublisher` and `LibraryVaultChapterPublisher` stay separate and keep separate active-target, publish-result, and workflow failure types. Observable service result semantics are aligned at the coarse success/failure level, including Capture-specific zero-success publish failure, and both publishers have parallel boundary tests. Manifest publish transaction mechanics are already shared: root manifest reads and identity validation, manga manifest path resolution, manga/root manifest writes, root pointer and summary updates, and rollback after root publish failure. Prepared chapter and cover upload mechanics are shared through `VaultContentUploader`, while workflow-specific WebDAV wrappers remain separate behind the narrow upload port. Service-level transfer job finalization and sanitized failure detail encoding are shared for Add to Vault workflows through `AddToVaultTransferFinalizer`, which owns terminal transfer state, counts, failure reason, completion timestamps, and detail JSON. Per-success Add to Vault refresh timing is shared through `AddToVaultIndexRefresher`: after each successful one-chapter publish, the configured Vault Catalogue is refreshed and the refreshed Vault Manga id is resolved before persisting an Import Target Hint. Local-to-Vault Import and Library-to-Vault Capture keep per-chapter refresh rather than batched refresh in v1. `LocalVaultImportService` and `LibraryVaultCaptureService` keep their public result mapping separate. `LibraryVaultChapterPublisher` owns Capture-specific manifest mutation inputs, target validation, and replacement cache invalidation, while `LibraryVaultChapterStager` owns Capture-specific downloaded-page copy/extraction, source page fetching, tall-image splitting, canonical CBZ writing, validation, and cover acquisition. Capture should not consume the local-import stager until shared lower-level page-to-CBZ mechanics can be extracted without merging workflow-specific source/download acquisition policy. Capture uses `LibraryVaultCaptureWebDav` as its workflow-specific transport wrapper; adopting `VaultWebDavClient` or a broad shared transport interface is deferred until transport behavior can be verified independently from capture publishing behavior. Shared scanner/stager mechanics, staged promotion beyond current manifest rollback, and result abstraction remain deferred. Full publisher tests currently cover successful create-new publish, replacement publish with cache invalidation, unconfirmed duplicates as per-chapter failures, root publish rollback, root identity mismatch, missing target pointer, and no empty Vault Manga after pre-publish failures; future sharing should preserve those semantics. `LocalVaultImportResult` and `LibraryVaultCaptureResult` cleanup and any shared result abstraction should wait until finer-grained Import and Capture result semantics are proven aligned beyond the coarse zero-success failure behavior.
...
Implementation debt: after Local-to-Vault Import and Library-to-Vault Capture both use one-chapter publish workflows with visible transfer results, continue extracting duplicated app-layer publishing mechanics into shared helpers or services. Root manifest reads and identity validation, manga manifest path resolution, root pointer updates, rollback after root publish failure, prepared content and cover upload, transfer job finalization, sanitized failure detail encoding, and per-success local index refresh are already shared for the two one-chapter publishers. Remaining shared code candidates include staged promotion beyond current manifest rollback.
