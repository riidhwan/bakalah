...
- For local agent release preparation, run only metadata checks that do not require GitHub CI context or remote GitHub API access. Do not run the release metadata workflow's remote duplicate-release check locally; CI verifies existing tags and GitHub Releases after the pull request is opened.
